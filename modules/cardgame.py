...
class CardGame(commands.Cog):
    ...
    async def card_daily(self, ctx):
        ...
        if today == lastdaily:
            ...
            h, m = divmod(m, 60)
            return await ctx.send("Wait another %sh %sm before using daily again..." % (h, m,))
        ...
        await ctx.send("Given character **%s!**" % character_loli.replace('_', ' ').title())
    ...
    async def card_sell(self, ctx, num: int):
        ...
        await ctx.send("%s, type `yes` to sell **%s** for %s" % (author.mention, cardname_en, "￥{:,}".format(cardprice)))
        ...
        await ctx.send("Sold %s for %s" % (cardname_en, "￥{:,}".format(cardprice)))
